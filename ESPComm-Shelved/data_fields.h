...
enum FIELD_TYPE : int8_t
{
	REMOVE = -1, NONE, RADIO, TEXT, SUBMIT, CHECKBOX, PASSWORD, SELECT, TEXTAREA
};
...
class UINT_Datafield : public DataField
{
	...
	String GenerateHTML() { return DataField::GenerateHTML(); }
...
class UINT8_S_Datafield : public DataField
{
	...
	bool SetFieldValue( const String & );
	const String GetFieldValue(){ return setting->ToString(); }
	String GenerateHTML() { DataField::GenerateHTML(); }
...
{
	public:
	SSID_Datafield(  unsigned int address, const String &fieldLabel = "", const String &fieldName = "", bool newLine = true ) :
	DataField( address, FIELD_TYPE::SELECT, fieldLabel, fieldName, "", newLine )
	{
	}
	
	bool SetFieldValue( const String & ){ return true; } //Do nothing here
	const String GetFieldValue(){ return WiFi.SSID(); }
